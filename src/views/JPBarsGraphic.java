...
public class JPBarsGraphic extends JPanel{
	...
	public JPBarsGraphic(String[] title,String[] nombresEjes,String[] nombresEjeX,int[]valores){
		init(title, nombresEjes, nombresEjeX, valores);

	}
	public void init(String[] title, String[] nombresEjes, String[] nombresEjeX, int[] valores) {
		removeAll();
...
//		revalidate();
	...
	private void drawAxes(Graphics2D graphics2d) {
		...
		graphics2d.drawString(namesOfAxes[0], X1_Axis_Y-((int)(getWidth()*(0.02+0.05))), Y1_Axis_Y+(Y2_Axis_Y-Y1_Axis_Y)/2);
	...
	private void drawDivisionsInTheAxisY(Graphics2D graphics2d) {
		for (int i = 0; i < NamesAxisY.size(); i++) {
			...
			graphics2d.drawString(toFormatInt(NamesAxisY.get(i)), X2_DivisionLine-((int)(getWidth()*(0.012*2.5))), Y1_DivisionLine-2);
		}
	...
	private void drawBars(Graphics2D graphics2d) {
		...
		for (int i = 1; i <= namesAxisX.length; i++) {
			...
			graphics2d.fill3DRect(X1_DivisionLine,Y2_Axis_Y-point, (int)((maxValX-X2_Axis_Y)/(namesAxisX.length+1))/3,
					Y2_Axis_Y-(Y2_Axis_Y-point), false);
		}
	}
	private int calculatePoint(int value) {
		return value*(Y2_Axis_Y-maxValYFinal)/maxDataValY;
	}
	private void addLebels(Graphics2D graphics2d){
		int point;
		for (int i = 1; i <= namesAxisX.length; i++) {
			X1_DivisionLine=X2_Axis_Y+((((maxValX-X2_Axis_Y)/(namesAxisX.length+1))*(i)));
			point = calculatePoint(values[i-1]);
			graphics2d.setColor(axesColor);
			graphics2d.drawString(""+toFormatInt(values[i-1]), X2_Axis_Y+((((maxValX-X2_Axis_Y)/(namesAxisX.length+1))*(i))),
					Y2_Axis_Y-point);
		}
	}
